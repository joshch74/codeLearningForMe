...
//cgv와 영화제목, 상영시간을 테이블에 넣어주는 메소드
	public void name_time(String cgv,String title, String time) {
		String sql = "INSERT INTO SCREEN_THEATER(S_SCHEDULE_TIME,S_SEAT_CNT,m_name,T_NAME,SEATNUM) VALUES(?,?,?)";
		try {
			pstm = conn.prepareStatement(sql);
	...
	public void name_time(String m_time,int ticketNum, String MovieChoice, String cgvCode, String sits) {
		String sql = "INSERT INTO SCREEN_THEATER(S_SCHEDULE_TIME,S_SEAT_CNT,m_name,T_NAME,SEATNUM) VALUES(?,?,?,?,?)";
		try {
			pstm = conn.prepareStatement(sql);
			pstm.setString(1, m_time);
			pstm.setInt(2, ticketNum);
			pstm.setString(3, MovieChoice);
			pstm.setString(4, cgvCode);
			pstm.setString(5, sits);
			rs = pstm.executeQuery();
...
//	----------------------------------------MovieChoiceView()메소드--------------------------------------------------------------
	...
	public void payCheck() {
		String sql = "SELECT * FROM SCREEN_THEATER WHERE userid=?";
