...
//			Xpath : /html/body/div[2]/div[3]/div[2]/div[1]/div/div[2]/ul/li[?]/div/ul/li[?]/a
	...
	void find (String cgvCode, String city_view, String MovieChoice){
		String id = "webdriver.chrome.driver";
		String path = "C:/chromedriver.exe";
		MovieDAO mdao = new MovieDAO();
		System.setProperty(id, path);
...
//			System.out.println(str.equals(MovieChoice));
			if (str.equals(MovieChoice)) {
				flag = true;	
			}
		}
		if(flag==true) {
			for (WebElement element1 : listTime) {
				...
				if (!time.contains("상영중")) {
					if (time.equals("") || time == null) {
						...
						System.out.println(time+"\n");
						mdao.name_time(cgvCode,MovieChoice,time);						
						}
					}	
				//title_find,time_find,serial을 어디에다가 넣어야 할까?
				}
		}else {
			System.out.println("현재 이 영화는 상영 시간대가 다 끝난 상태입니다.");
			new TicketView();
		}
		driver.close();
	}
